package sort;

/**
 * 力扣 147. 对链表进行插入排序
 * 给定单个链表的头 head ，使用 插入排序 对链表进行排序，并返回 排序后链表的头 。
 * <p>
 * 插入排序 算法的步骤:
 * <p>
 * 1.插入排序是迭代的，每次只移动一个元素，直到所有元素可以形成一个有序的输出列表。
 * 2.每次迭代中，插入排序只从输入数据中移除一个待排序的元素，找到它在序列中适当的位置，并将其插入。
 * 3.重复直到所有输入数据插入完为止。
 * 下面是插入排序算法的一个图形示例。部分排序的列表(黑色)最初只包含列表中的第一个元素。每次迭代时，从输入数据中删除一个元素(红色)，并就地插入已排序的列表中。
 * <p>
 * 对链表进行插入排序。
 */
public class LC147 {
    public static void main(String[] args) {
        int[] array = {-1,5,3,4,0};
        ListNode head = init(array);
        ListNode sortedHead = insertionSortList(head);
        while (sortedHead != null) {
            System.out.println(sortedHead.val);
            sortedHead = sortedHead.next;
        }
    }


    public static ListNode insertionSortList(ListNode head) {
        ListNode sortedHead = new ListNode();
        if (head == null) {
            return sortedHead;
        }
        sortedHead.val = head.val;

        while (head.next != null) {
            ListNode temp = sortedHead;
            ListNode preTemp = null;
            ListNode insertNode = head.next;
            while (temp != null) {
                if (insertNode.val > temp.val) {
                    preTemp = temp;
                    temp = temp.next;

                    if (temp == null) {
                        insertAfter(preTemp, insertNode);
                    }
                } else {
                    if (preTemp == null) {
                        sortedHead = insertBefore(sortedHead, insertNode);
                        break;
                    } else {
                        insertAfter(preTemp, insertNode);
                        break;
                    }
                }
            }
            head = head.next;
        }

        return sortedHead;
    }

    private static void insertAfter(ListNode sortedNode, ListNode insertNode) {
        ListNode newNode = new ListNode();
        ListNode next = sortedNode.next;

        newNode.val = insertNode.val;
        sortedNode.next = newNode;
        newNode.next = next;
    }

    private static ListNode insertBefore(ListNode sortedHead, ListNode insertNode) {
        ListNode newNode = new ListNode();
        newNode.val = insertNode.val;
        newNode.next = sortedHead;
        return newNode;
    }

    private static ListNode init(int[] array) {
        ListNode head = new ListNode();
        head.val = array[0];
        ListNode nowNode = head;
        for (int i = 1; i < array.length; i++) {
            ListNode node = new ListNode();
            node.val = array[i];
            nowNode.next = node;
            nowNode = node;
        }
        return head;
    }
}
